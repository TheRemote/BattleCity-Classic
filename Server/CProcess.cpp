	...
	try {
		...
		switch((unsigned char)TheData[0])  //Packet ID
		{
			...
			case cmClickPlayer:
				...
				break;
			case cmCycleTank:
				ProcessCycleTank(Index);
				break;
			case cmPrivateMessage:
				ProcessPrivateMessage(Index, TheData[1], &TheData[2]);
				break;
		}//(switch)
...
void CProcess::ProcessBuild(char *TheData,int Index)
{
	if (p->Player[Index]->Mayor && p->Player[Index]->State == State_Game && p->Player[Index]->isDead == false)
	{
		...
		if (p->City[p->Player[Index]->City]->canBuild[data->type - 1] == 1 || p->Player[Index]->isAdmin == 2)
		{
			...
			if (p->City[p->Player[Index]->City]->Orbable == false) 
			{
				int count = p->Build->GetOrbBuildingCount(p->Player[Index]->City);
				if (count >= 11) p->City[p->Player[Index]->City]->Orbable = true;
			}
	...
	if (p->Player[Index]->isAdmin)
	{
			...
			p->PlatformCaseCompare(p->Player[Index]->Name.c_str(), "Weebo") == 0)
		{
			if (p->Account->CheckAccount(adminedit->User) == 1)
			{
				...
				std::ostringstream MonthlyTop20Convert;
				MonthlyTop20Convert << adminedit->MonthlyPoints;
				DMLString += MonthlyTop20Convert.str();
				...
				for (int i = 0; i < MaxPlayers; i++)
				{
					...
					{
						if (adminedit->IsAdmin == 1) adminedit->IsAdmin = 2; else adminedit->IsAdmin = 1;
						...
						p->Player[i]->MonthlyPoints = adminedit->MonthlyPoints;
...
void CProcess::ProcessCycleTank(int Index)
{
	int CurrentTank = p->Player[Index]->Tank;
	if (CurrentTank == p->Player[Index]->Tank1 && p->Player[Index]->Tank1 > 0)
	{
		if (p->Player[Index]->Tank2 > 0)
		{
			CurrentTank = p->Player[Index]->Tank2;
		}
		else
		{
			CurrentTank = 0;
	...
	else if (CurrentTank == p->Player[Index]->Tank2 && p->Player[Index]->Tank2 > 0)
	{
		if (p->Player[Index]->Tank3 > 0)
		{
			CurrentTank = p->Player[Index]->Tank3;
		}
		else
		{
			CurrentTank = 0;
	...
	else if (CurrentTank == p->Player[Index]->Tank3 && p->Player[Index]->Tank3 > 0)
	{
		if (p->Player[Index]->Tank4 > 0)
		{
			CurrentTank = p->Player[Index]->Tank4;
		}
		else
		{
			CurrentTank = 0;
	...
	else if (CurrentTank == p->Player[Index]->Tank4 && p->Player[Index]->Tank4 > 0)
	{
		if (p->Player[Index]->Tank2 > 0)
		{
			CurrentTank = 0;
	...
	else if (CurrentTank == 0 && p->Player[Index]->Tank1 > 0)
	{
		CurrentTank = p->Player[Index]->Tank1;
	}

	p->Player[Index]->Tank = CurrentTank;

	char packet[3];
	packet[0] = Index;
	packet[1] = CurrentTank;
	p->Send->SendAllBut(-1, smUpdateTank, packet, 2);
}

void CProcess::ProcessPrivateMessage(int Index, int Player, char *Message)
{
	if (p->Player[Index]->Member == 1 || p->Player[Index]->isAdmin == 2)
	{
		char SendString[255];
		SendString[0] = Index;
		strcpy(&SendString[1], Message);

		if (p->Player[Player]->State == State_Game)
		{
			p->Winsock->SendData(Player, smPrivateMessage, SendString);
		}
	}
}