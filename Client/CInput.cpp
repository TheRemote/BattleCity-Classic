...
void CInput::ProcessKeys(char buffer[256])
{
	...
	{
		if (p->Tick > p->InGame->timeChangeTank)
		{
			p->InGame->timeChangeTank = p->Tick + 1000;
			p->Winsock->SendData(cmCycleTank, "");
		}
	}
				...
				if (p->Collision->CheckBuildingCollision(Xloc, Yloc) == 0 || p->Player[p->Winsock->MyIndex]->isAdmin == 2)
				{
					if (p->Build->inRange() == 1)
					{
						if (Xloc > 0 && Xloc < 510 && Yloc > 0 && Yloc < 510)
						{
							if (p->InGame->IsBuilding == 0)
							{
								p->InGame->setCash(p->InGame->Cash - 2000000);
							}
							else
							{
								p->InGame->setCash(p->InGame->Cash - 500000);
							}
							sCMBuild b_packet;
							b_packet.x = Xloc;
							b_packet.y = Yloc;
	...
	if (mouse_state.rgbButtons[1] & 0X80) //Right Click
	{
		...
		if (X >= 0 && X <= p->Draw->MaxMapX && Y >= 0 && Y <= p->Draw->MaxMapY)
		{
			...
			for (int i = 0; i < MaxPlayers; i++)
			{
				if (foundplayer == 0)
				{
					int CheckX = (int)(p->Player[me]->X - p->Player[i]->X + p->Draw->PlayerOffsetX);
					int CheckY = (int)(p->Player[me]->Y - p->Player[i]->Y + p->Draw->PlayerOffsetY);

					
					if (p->Collision->RectCollision(CheckX, CheckY, 48, 48, 0, p->Input->LastMouseX, p->Input->LastMouseY, 1, 1, 0))
					{
						foundplayer = 1;
