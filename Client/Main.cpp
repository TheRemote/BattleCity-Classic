						...
						if (Game.InGame->ChatLine.length() > 0)
						{
							if (Game.Player[Game.Winsock->MyIndex]->isAdmin == 2)
							{
								...
								if (strcmp(temp.c_str(), "/g") == 0) //Global message
								{
									...
									tmpString = "(Global) " + Game.Player[Game.Winsock->MyIndex]->Name + ": " + SendString;
									Game.InGame->AppendChat(tmpString, RGB(255, 165, 0));
									Game.InGame->ChatLine.clear();
								}
								else if (strcmp(temp.c_str(), "/pm") == 0) //Private message
								{
									if (Game.Player[Game.Winsock->MyIndex]->Member == 1)
									{
										parser >> temp;
										std::cout << temp << std::endl;
										int i = Game.InGame->FindPlayer(temp);
										if (i != -1 && i != Game.Winsock->MyIndex)
										{
											if (Game.InGame->ChatLine.length() > temp.length() + 5)
											{
												Game.InGame->AppendChat("(PM to " + Game.Player[i]->Name + ") " + Game.InGame->ChatLine.substr(temp.length() + 5), RGB(255, 255, 255));
												char SendString[255];
												SendString[0] = i;
												strcpy(&SendString[1], Game.InGame->ChatLine.substr(temp.length() + 5).c_str());
												Game.Winsock->SendData(cmPrivateMessage, SendString);
											}
											else
											{
												Game.InGame->AppendChat("Enter a message!", RGB(255, 255, 255));
											}
											Game.InGame->ChatLine.clear();
										}
										else
										{
											Game.InGame->AppendChat("Player not found!", RGB(255, 255, 255));
											Game.InGame->ChatLine.clear();
										}
									}
									else
									{
										Game.InGame->AppendChat("You are not a member!", RGB(255, 255, 255));
										Game.InGame->ChatLine.clear();
									}
								}
								else //Chat Line
							...
							else
							{
								...
								if (strcmp(temp.c_str(), "/pm") == 0) //Private message
								{
									if (Game.Player[Game.Winsock->MyIndex]->Member == 1)
									{
										parser >> temp;
