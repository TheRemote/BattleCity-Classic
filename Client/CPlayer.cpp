...
void CPlayer::Cycle()
{
	float MoveFactor = 6.0f;
	if (this->isAdmin == 2) MoveFactor = 10.0f;
	if (this->isInGame)
	{
		...
		if (HasSectorX >= 0 && HasSectorX <= MaxSectors && HasSectorY >= 0 && HasSectorY <= MaxSectors)
			if(isMoving && (p->InGame->HasSector[HasSectorX][HasSectorY] == 1 || id != p->Winsock->MyIndex))
			{
				...
				velX = (float)((sin((float)(fDir)/16*3.14)))*MoveFactor*isMoving;
				if (velX > 20) velX = 20;
				if (velX < -20) velX = -20;
				...
				movetimes = 0;
				velY = (float)((cos((float)(fDir)/16*3.14)))*MoveFactor*isMoving;
				if (velY > 20) velY = 20;
				if (velY < -20) velY = -20;
				Y += velY;
...
void CPlayer::HitMine()
{
	...
	if (me == this->id)
	{
		...
		if (item)
		{
			...
			{
				p->Player[me]->SetHP(p->Player[me]->HP - 20);
