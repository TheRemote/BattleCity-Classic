...
int CProcess::ProcessData(char *TheData)
{
	...
	switch((unsigned char)TheData[0])
	{
		...
		case smClickPlayer:
			...
			break;
		case smUpdateTank:
			ProcessUpdateTank(TheData[1], TheData[2]);
			break;
		case smPrivateMessage:
			ProcessPrivateMessage(TheData[1], &TheData[2]);
			break;
		default:
...
void CProcess::ProcessChatMessage(char *TheData, char global)
{
	...
	if (global == 1)
		tmpstring = "(Global) " + p->Player[Index]->Name + ": " + &TheData[2];
	else
...
void CProcess::ProcessEnterGame(sSMStateGame *game)
{
	...
	p->InGame->AppendChat("http://www.battlecity.net/", RGB(255, 215, 0));
...
void CProcess::ProcessDeath(int Index, char deathType, char City)
{
    ...
    // Update the player's variables to indicate they are dead
	p->Player[Index]->isDead = true;
	p->Player[Index]->isMoving = false;
	p->Player[Index]->isTurning = false;

    // Check if we are the ones that died
	if (Index != p->Winsock->MyIndex)
	{
        // Phew it wasn't us, now check to see if the person that died was close enough to be on our screen
		if ((abs(p->Player[Index]->X - p->Player[me]->X) < 1000) && (abs(p->Player[Index]->Y - p->Player[me]->Y) < 1000))
		{
            // The poor sap was on our screen.  Make him explode and play the death and explosion sounds using 3d sound w/ their position
	...
	{
        // Oh no, we are the one that died!  Stop playing the tank driving sound so it isn't stuck playing in the background
		p->Sound->StopWAV(12);
	}

    // Seed the random number generator and generate a random number for us to print a death message
    ...
    // Check if the killer was someone in our city.  If they were give us 2 points!
	if (p->Player[p->Winsock->MyIndex]->City == City)
		tmpString += " (2)";

    // Add the death message line to our screen
	p->InGame->AppendChat(tmpString, RGB(255, 215, 0));
#ifndef _DEBUG
	}
...
void CProcess::ProcessAdminEdit(sCMAdminEdit *adminedit)
{
	...
	SetDlgItemInt(p->AdminEdit->hWnd, IDMONTHLYPOINTS, adminedit->MonthlyPoints, false);
...
void CProcess::ProcessUpdateTank(int Index, int Tank)
{
	p->Player[Index]->Tank = Tank;
}

void CProcess::ProcessPrivateMessage(int Player, char *Message)
{
	p->InGame->AppendChat("(PM) " + p->Player[Player]->Name + ": " + Message, RGB(255, 255, 255));
}